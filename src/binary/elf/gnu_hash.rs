...
pub struct GnuHash<'process> {
    nbuckets: u32,
    symindex: usize,
    maskwords: usize, // how many words our bloom filter mask has
    shift2: u32,
    maskbits: u32,
...
impl<'process> GnuHash<'process> {
    pub fn new (hashtab: *const u32, total_dynsyms: usize) -> GnuHash<'process> {
	    ...
	    let nbuckets = *hashtab;
            ...
            let maskwords = *hashtab.offset(2) as usize;
            ...
            GnuHash { 
                ...
                maskwords: maskwords,
                shift2: shift2,
                maskbits: 64, // because we don't have a compile time sizeof yet...
    ...
    #[inline(always)]
    fn lookup(&self, hash: u32, symbol: &str, bias: u64, strtab: &'process strtab::Strtab, symtab: &'process[sym::Sym]) -> Option<u64> {
